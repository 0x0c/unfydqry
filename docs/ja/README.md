...
| `indexBatch([IndexItem(id, text), …])` | 複数の `(id, text)` ドキュメントを単一トランザクションでインデックスする。大量バッチでは `index` の逐次呼び出しより大幅に速い。処理件数を返す。 |
| `removeBatch([id, …])` | 複数の id を単一トランザクションで削除する。存在しない id はスキップ。処理件数を返す。 |
...
| `indexRecordsBatch([RecordIndexItem(recordId, [FieldValue(slot, text), …]), …])` | 複数レコードを単一トランザクションで upsert。all-or-nothing で、いずれかの `recordId` や `slot` が不正なら何もインデックスしない。処理件数を返す。 |
...
- **複数フィールドのレコード**を `indexRecordsBatch`(1レコードにつき名前＋よみ)で単一トランザクションでまとめてインデックスし、`searchRecords` で検索。各結果行にどのフィールドが一致したかを表示し、record-layer API を実演する。
- **バッチ操作**: 一括追加 / 一括削除ボタンで、2つ目のレコード群を単一トランザクションで追加・削除する(`indexRecordsBatch` / `removeBatch`)。
...
Flutter サンプル(`flutter/example`)も同じ UX を踏襲し、バッチ追加/削除ボタンを含む。
