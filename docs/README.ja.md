...
- **既定の検索**は SQLite FTS5 + trigram を `bm25` でランキング。`substring` と `prefix` アルゴリズムも選択可能
...
│   ├── src/search/             差し替え可能な検索アルゴリズム(trigram_bm25/substring/prefix)
...
補足:
- ランキングされるのは `trigram_bm25` のみ。trigram は3文字未満のクエリを照合できないため、その場合は自動的に substring `LIKE`(スコア `0.0`)へフォールバックする
- `substring` と `prefix` は非ランキング。格納順で定数スコア `0.0` を返すので、件数は `limit` で制限する
